 ...
 * HISTORY:
 * Last edited: Aug 28 09:25 2008 (edgrif)
...
int writeSequence (FILE *fil, int *conv, char *seq, char *id, char *desc, int len)
{
  int i ;
  ...
  for (i = 0 ; i < len ; ++i)
	...
	fputc ('\n', fil) ;
      ...
      if (conv[(unsigned char)seq[i]] > 0)
	fputc(conv[(unsigned char)seq[i]], fil) ;
      else
	{
  ...
  for (i = 0 ; fgets(line, 1023, fil) && i < nsymb ; ++i)
    {
      ...
      if (p && conv[(unsigned char)*p] == symb[i])
	{ ++p ; while (*p == ' ' || *p == '\t' || *p == '\n') ++p ; }
      for (j = 0 ; *p && j < 128 ; ++j)
