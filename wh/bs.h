 ...
 * Exported functions: 
 * HISTORY:
 * Last edited: Aug 12 14:05 2004 (edgrif)
 * Created: Thu Aug 26 17:54:10 1999 (fw)
 * CVS info:   $Id: bs.h,v 1.54 2012-10-24 08:53:35 edgrif Exp $
 *-------------------------------------------------------------------
 */
#ifndef ACEDB_BS_H
#define ACEDB_BS_H
...
typedef struct ObjStruct *OBJ ;	/* public opaque type */

typedef struct BShowStruct *BS;	/* public opaque type */
...
/* Fuse two objects into one, the effect is that old is thrown away, and new */
/* becomes old overlaid with new.                                            */
BOOL bsFuseObjects (KEY old, KEY new) ;

/* Several sorts of dumps */
void bsDump        (OBJ obj) ;             /* dumps on stdout in debug format */

void bsAceDump     (OBJ obj, ACEOUT dump_out, BOOL isInternal) ;  /* ace dump a B Tree  */
 ...
 * (when you start a series of operations on a BS never start with a _bsDown, _bsRight). */
...
BOOL bsGetText     (OBJ obj, KEY target, char **cpp) ; /* converts all into text, including Int, Float, DateType and KEYs - text is readonly! */
...
BSMARK bsHandleMark (OBJ obj, BSMARK mark, STORE_HANDLE hh) ; /* allocates mark on hh */
...
/* Some functions to examine the model in relation to the current object. */
BS bsModelRoot(OBJ obj) ;
BS bsModelCurrent(OBJ obj) ;
BS bsModelRight(BS bsm) ;
BS bsObjectCurrent(OBJ obj) ;
BOOL bsModelMatch (BS bs, BS *bsm) ;
KEY bsModelKey(BS bsm) ;


/* Some debugging aids. */
void bsModelShowNeighbours(BS bsm, int depth) ; /* display node & neighbours for debugging. */
void bsShowNeighbours(BS bs, int depth) ; /* display node & neighbours for debugging. */
char *bs2Text (BS bs, BOOL dont_expand_text, BOOL is_model) ;




/* These really shouldn't be here, avoid using if at all possible.... */
int cacheSaveAll(void) ;
int cacheStatus (int *used, int *locked, int*known, int *modified, 
		 int *nc2rp, int *nc2wp, int *nccr, int *nccw) ;
 
/* for status */
void BSstatus (int *used, int *alloc, int *mem) ;
