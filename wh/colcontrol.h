 ...
 * Exported functions:
 * HISTORY:
 * Last edited: May 13 09:49 2008 (edgrif)
 * * Oct  7 10:31 1998 (edgrif): Removed ACEDB defines, now some acedb
 *              fields are permanently in the structures.
 *-------------------------------------------------------------------
 */

/* $Id: colcontrol.h,v 1.49 2008-05-22 11:33:36 edgrif Exp $  */

#ifndef ACEDB_COLCONTROL_H
#define ACEDB_COLCONTROL_H

#include "acedb.h"
#include "bs.h"			/* for public OBJ type */
...
typedef struct ColControlStruct
{
  ...
  STORE_HANDLE handle; /* storage here */
...
struct MapControlStruct
{
  ...
  void (*keyboard)(int k, int modifier);
...
struct InstanceStruct
{
  ...
  /* displayed = TRUE means instance should be drawn _if_ zoom level/display width etc is correct.
   * drawn = TRUE means column actually was drawn, these are set by instance callbacks. */
  BOOL displayed ;
  BOOL drawn ;
...
COLINSTANCE controlInstanceFind(COLCONTROL control, char *name) ;
