 ...
 * CVS info:   $Id: acediffsorted.c,v 1.13 2004-08-21 21:59:36 mieg Exp $
 *-------------------------------------------------------------------
 */
   ...
   replace A-class object data with pointer into input files */
{
  ...
  while (freeread (buf->in)) { /* read only one object */ 
	...
	isAObj= TRUE; isLongText= TRUE; isDNA= FALSE;
      } else if (strcmp(word,"DNA")==0) {
	isAObj= TRUE; isLongText= FALSE; isDNA= TRUE;
      } else isAObj= FALSE;
