 ...
 * Exported functions:
 * HISTORY:
 * Last edited: Aug 31 16:02 2006 (edgrif)
 ...
 * CVS info:   $Id: acesocket.c,v 1.21 2006-08-31 15:05:47 edgrif Exp $
 *-------------------------------------------------------------------
 */
...
/* Global data, if you add any more you should put all this in a struct
 * and add accessor functions etc. */

BOOL acesock_sigterm_received_G = FALSE ;		    /* Used for signal handling. */


/* local statics, please don't add any more unless you _absolutely_ have to. */
		      ...
		      AceSocketServerRequestRoutine serverRequestFunc, void *serverData) 
{
  ...
  while (TRUE)
    ...
    shutdownCode:
      if (shuttingDown || acesock_sigterm_received_G)
	{
	  ...
	  if (serverRequest == ACESOCK_SHUTFORCE
	      ...
	      || serverRequest == ACESOCK_EXIT
	      || serverRequest == ACESOCK_TERM)
	    {	
		...
		Close(listenfd) ;
	      currTarget = ACESOCK_SERVER ;
	      if (serverRequest == ACESOCK_TERM)
		currRequest = ACESOCK_TERM ;
	      else
		currRequest = ACESOCK_EXIT ;
      ...
      /*      no clients => use server time out                              */
      ...
      else
	{
      ...
      if (nready == 0 || acesock_sigterm_received_G)
	{
	  if (acesock_sigterm_received_G)
	    {
	      /* Somewhere we received a sigterm so exit asap */
	      serverRequest = ACESOCK_TERM ;
	    }
	  ...
	  if (connfd >= 0)
	      ...
	      else
		{
		  ...
		  /* Accept the connection and make a new client.        */
	 	  if (setsockopt(connfd, SOL_TCP, TCP_NODELAY, &one, sizeof(one)) < 0)
		    messdump("new client on socket %d - cannot set TCP_NODELAY: errno %d\n",
		      connfd, errno);
	  ...
	  switch (client->state)
	      ...
	      if (currTarget == ACESOCK_SERVER)
		{
		  ...
		  if (currRequest == ACESOCK_SHUTFORCE)
		    ...
		    {
		      /* We don't kill this client, just signal the server not to allow
		       * any new connections. The server then quits when the last client quits. */
		      shuttingDown = TRUE ;
		      serverRequest = currRequest ;
...
/* Sets a global which we use to try to shut down everything cleanly in the
 * event of receiving a TERM signal. */
void aceTransportRequestShutdown(void)
{
  acesock_sigterm_received_G = TRUE ;
}
