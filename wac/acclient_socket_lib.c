 ...
 * CVS info:   $Id: acclient_socket_lib.c,v 1.5 2003-07-23 20:45:47 mieg Exp $
 *-------------------------------------------------------------------
 */

#include <stdio.h>
#include <string.h>
#if defined(NEXT) || defined(HP) || defined(MACINTOSH) || defined(MAC_X)
...
static AceConnStatus AceConnCreate(AceConnection *connection, char *server_netid, int server_port,
...
static AceConnection aceconnCreate(char *server_netid, int server_port,
			    char *userid, char *passwd, int timeout) ;
static void aceconnDestroy(AceConnection connection) ;
...
static void aceconnSetErrMsg(AceConnection connection, char *msg1, char *msg2 )
{
/*
...
static AceConnStatus aceconnSocketRead(AceConnection connection)
{
  ...
  if (status == ACECONN_OK)
      ...
      if (req->done_header == TRUE)
	{
	  ...
	  if (!req->message)
	    {
	      req->message = malloc(req->proto_hdr.length) ;
...
static AceConnMsg messageCreate(void)
{
  AceConnMsgRec *msg = (AceConnMsgRec *)calloc(sizeof(AceConnMsgRec),1) ;
...
static void messageGetEncore(AceConnMsg msg_in)
{
  ...
  if (msg->message != NULL)
	...
	{
	msg->encore_message.str = malloc(msg->mBytesRequested);
...
static char *convertMD5toHexStr(unsigned char digest[])
{
  ...
  digest_str = malloc(MD5_HEX_HASHLEN) ;
