 ...
 * Exported functions:
 * HISTORY:
 * Last edited: May 22 10:31 2008 (edgrif)
 ...
 * CVS info:   $Id: colcontrol.c,v 1.2 2008-05-22 11:42:16 edgrif Exp $
 *-------------------------------------------------------------------
 */
...
static void controlKybd(int key, int modifier);
...
/* Searches control for an instance with the given name, returns NULL if instance not found.
 * NOTE this relies on the hard coded names for columns, e.g. "Loci" not being changed. */
COLINSTANCE controlInstanceFind(COLCONTROL control, char *name)
{
  COLINSTANCE instance = NULL ;
  int i ;

  for (i = 0 ; i < arrayMax(control->instances) ; i++)
    {
      COLINSTANCE tmp = (COLINSTANCE)arr(control->instances, i, COLINSTANCE) ;
