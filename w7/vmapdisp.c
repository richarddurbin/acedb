 ...
 * Exported functions:
       vMapDisplay
 * HISTORY:
 * Last edited: May  6 10:51 2003 (edgrif)
 ...
 * CVS info:   $Id: vmapdisp.c,v 1.35 2003-05-06 13:13:50 edgrif Exp $
 *-------------------------------------------------------------------
 */
...
static void vMapPick (int box, double x, double y, int modifier_unused) ;
static void vMapKbd (int k, int modifier_unused) ;
...
static void vMapDrawMiniChromBands (LOOK genericLook, float *offset, MENU *unused) ;
static void vMapDrawMainMarkers (LOOK genericLook, float *offset, MENU *unused) ;
static void vMapDrawAnyInterval (LOOK genericLook, float *offset, MENU *unused) ;
static void vMapDrawAnyIntervalNames (LOOK genericLook, float *offset, MENU *unused) ;
static void vMapDrawChromBands (LOOK genericLook, float *offset, MENU *unused) ;
static void vMapDrawScale (LOOK genericLook, float *offset, MENU *unused) ;
static void vMapDrawAnyLocus (LOOK genericLook, float *offset, MENU *unused) ;
static void vMapDrawOrderedGenes (LOOK genericLook, float *offset, MENU *unused) ;
...
BOOL vMapDisplay (KEY key, KEY from, BOOL isOldGraph, void *unused)
{
  ...
  look->map = mapCreate (vMapDrawVoid) ;        /* the map's redraw func */
...
static void vMapPick (int box,  double x , double y, int modifier_unused) 
{
  ...
  else if (box == look->map->thumb.box - 1)
   { /* coordinates are realtive to thumb -1 .box */
     graphFitBounds (&mapGraphWidth, &mapGraphHeight) ;
     if (y < 1) /* page up */
	look->map->centre -=  1.5 * look->map->thumb.halfwidth/look->map->thumb.fac ;
      else if (y > mapGraphHeight - topMargin - 3.2) 
	look->map->centre +=  1.5 * look->map->thumb.halfwidth/look->map->thumb.fac ;
      else
	look->map->centre = WHOLE2MAP (look->map, y + topMargin + .2) ;
      vMapDraw (look, 0) ;
    }
  else if (box == look->map->thumb.box)
    {  /* coordinates are realtive to thumb.box */
       look->map->centre += 
	(y - look->map->thumb.halfwidth)/look->map->thumb.fac ;
