 ...
 * Exported functions:
 * HISTORY:
 * Last edited: May 13 13:45 2008 (edgrif)
 *-------------------------------------------------------------------
 */

/* $Id: gmapsubmapcol.c,v 1.10 2008-05-22 12:02:31 edgrif Exp $ */
...
int gMapSubMapOrder (void *a, void *b);
...
int gMapSubMapOrder (void *a, void *b)  /* for arraySort() call */
{
  GMAPSUBMAPSEG *seg1 = (GMAPSUBMAPSEG*)a, *seg2 = (GMAPSUBMAPSEG*)b ;
  float diff;
...
void *submapConvert(MAPCONTROL map)
{
  ...
  /* no segs */
  if ((cache = gMapCacheKey(map->key, "subs.empty")) &&
      (tempsegs = arrayGet(cache, SUBMAPTEMP, submapFormat)))
    return arrayCreate (1, GMAPSUBMAPSEG);
  ...
  if (arrayMax(tempsegs) == 0)
    {
      arrayMax(tempsegs) = 1;
      gMapCacheStore(map->key, "subs.empty", tempsegs, submapFormat);
    }
  arrayDestroy(tempsegs);
