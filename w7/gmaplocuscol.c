 ...
 * CVS info:   $Id: gmaplocuscol.c,v 1.41 2011-01-06 18:06:02 edgrif Exp $
 *-------------------------------------------------------------------
 */
...
/* Searches column instance for a locus/gene key, returns NULL if key not found in instance. */
GMAPSEG *gMapFindSeg(COLINSTANCE instance, KEY gene_key)
{
  GMAPSEG *seg = NULL ;
  GENESPRIV private = instance->private ;
  Array gene_segs = private->segs ;
  int i ;

  for (i = 0 ; i < arrayMax(gene_segs); i++)
    {
      GMAPSEG *tmp ;
      
      tmp = arrp(gene_segs, i, GMAPSEG) ;
...
/* Returns TRUE if the public name of gene_key object is same as pub_name. */
BOOL gmapGeneIsPub(KEY gene_key, char *pub_name)
{
  BOOL result = FALSE ;
  KEY pub_key ;

  pub_key = geneGetPubKey(gene_key) ;

  if (g_ascii_strcasecmp(name(pub_key), pub_name) == 0)
    result = TRUE ;

  return result ;
}

/* Mark a gene for actions like global maximum likelihood.... */
void gmapHighlightGene(int box)
{
  genesHighlightSeg(box) ;
...
{ 
  ...
  if (seg->key == look->selectKey && instance == control->activeInstance &&
    ...
    {
      KEY pub_tag = str2tag("Public_name") ;
      KEY public_name_key = KEY_NULL ;
      KEY gene_key = seg->key ;

      graphBoxDraw(box, BLACK, CYAN);

      if (!bIndexGetKey(gene_key, pub_tag, &public_name_key))
	{
	  public_name_key = gene_key ;
	  gene_key = KEY_NULL ;
	}

      strncpy(look->messageText, name(public_name_key), 100);
      if (gene_key)
	strncat(look->messageText, messprintf(" (%s)", name(gene_key)), 100);
  ...
  *offset += 1 ;		/* bump bug */
  if (!private->showOrdered)
      ...
      for (i = firstSeg ; i != lastSeg ; i += countDir)
	{
	  ...
	  if (!(seg->flag & FLAG_ANY_LOCUS) || !(seg->flag & FLAG_WELL_ORDERED))
	    continue;
	  ...
	  if ((n = bumpText (bump, showName, &x, &y, 1, TRUE)) && y < control->graphHeight-0.5)
	    {
	      if(nameLength < n)
	        n = nameLength;
	      ...
	      for (j = 0 ; j < arrayMax(private->clonedQuerys) ; j++)
		{ 
		  CLONEDQUERY *q = arrp(private->clonedQuerys,j,CLONEDQUERY) ;

		  if (q->colour == BLACK && 
		      q->keyset &&
		      keySetFind(q->keyset, seg->key, 0))
		    {
		      float old = graphLinewidth(UNDERLINE_WIDTH);
		      graphLine (*offset+x, y+0.3, 
				 *offset+x+nameLength*0.7, y+0.3) ;
		      graphLinewidth(old);
		    }
		}
  ...
  /* If there is too little space we simply don't draw the genes, there is no point
   * as they will all overlap, we need to record whether they are drawn for searches etc. */
  if (x < 6)
    {
  ...
  if (init)			/* why should this be empty? */
	...
	{ CLONEDQUERY *q = arrayp (private->clonedQuerys, 
	  ...
	  bsFlatten(init,2,flatA))
        for (i=0,j=0; i < arrayMax(flatA); i+=2)
	  { CLONEDQUERY *q = arrayp (private->clonedQuerys, 
				     arrayMax(private->clonedQuerys),
				     CLONEDQUERY);
