...
static void fpPick (int box,  double x , double y, int modifier_unused) ;
...
static void fpDoSelectBands (int box, double x_unused, double y_unused, int modifier_unused)
{
  int n , nn ;
  Array fp ;
  Array old ;
...
static void fpPickSwitch (int box, double x_unused, double y_unused, int modifier_unused)
{
  int n, a, b;
  GEL gel ;
...
static void fpMainLines(LOOK look_unused, float *offset, MENU *menu_unused)
{
  ...
  if (box == fpMap->cursor.pickBox)
   ...
   { /* coordinates are realtive to thumb -1 .box */
	...
	return ;
      ...
      if (nn)
	display(clone, (KEY)0, 
		!strcmp(name(pickDisplayKey(clone)), "DtPmapFingerprint") ? "TREE" : "") ;
      if (fpActiveGel != n)
...
/* column "Bands" */
/* This is of type MapColDrawFunc */
static void fpDrawGels(LOOK look_unused, float *offset,  MENU *menu_unused)
{ 
...
void fpDisplay (KEY clone)
{
  ...
  if (!graphExists(fpGraph))
	...
	return ;
      isBp = TRUE ;
      fpMap = mapCreate (fpDraw) ; /* the map's redraw func */
      mapAttachToGraph (fpMap) ;
...
BOOL fingerPrintDisplay (KEY clone, KEY from, BOOL reuse, void *unused)
{ fpDisplay (clone) ;
...
#else
  if (bsGetKey(Clone, _Sequence, &seqKey) &&
       (dna = fMapFindDNA (seqKey, &from, &to, TRUE)))
    {
#endif
