 ...
 * HISTORY:
 * Last edited: Jan  6 17:22 2011 (edgrif)
 * Created: Tue Nov 30 18:42:30 1993 (rd)
 * CVS info:   $Id: gmapdatacol.c,v 1.4 2011-01-06 18:06:01 edgrif Exp $
 *-------------------------------------------------------------------
 */
  ...
  if (seg->key == look->selectKey && 
      instance == control->activeInstance) 
    {
      ...
      *look->messageText = 0 ;
      ...
      gene_key = geneGetPubKey(seg->key) ;
      strcat (look->messageText, name(gene_key)) ;

      strcat (look->messageText, ": ") ;

      gene_key = geneGetPubKey(arr(seg->data->loci,0,KEY)) ;
      strcat (look->messageText, name(gene_key)) ;

      for (i = 1 ; i < arrayMax(seg->data->loci) ; ++i)
	{
	  gene_key = geneGetPubKey(arr(seg->data->loci,i,KEY)) ;
	  strcat (look->messageText, 
      ...
      instance == control->activeInstance)
    {
      ...
      *buf = 0 ;
      ...
      strncpy (buf, messprintf ("2point %s: %s %s: %s",
				name(gene_key), 
				name(data->public_name_1), 
				name(data->public_name_2), 
				data->type ? name(data->type) : "Simple distance" ),
	       100) ; /* TODO OK? */
