 ...
 *     
 * HISTORY:
 * Last edited: Mar  5 16:46 2009 (edgrif)
 * CVS info:   $Id: gmapconvert.c,v 1.45 2009-03-09 14:27:07 edgrif Exp $
 *-------------------------------------------------------------------
 */
...
BOOL gMapGetMultiPtData(MAPCONTROL map, KEY multi, STORE_HANDLE handle, MULTIPTDATA *ret)
{
  Array counts, loci;
  float y, min, max;
  OBJ obj;
...
BOOL gMapGet2PtData(MAPCONTROL map, KEY key, STORE_HANDLE handle, TWOPTDATA *ret) 
{
  ...
  if ((!bsGetKey(obj, type_tag_1, &data->loc1) || !bsGetKey(obj, type_tag_2, &data->loc2))
  ...
  else
    {
      ...
      public_key = str2tag("Public_name") ;

      data->public_name_1 = data->loc1 ;
      data->public_name_2 = data->loc2 ;

      /* Get public names for genes if they exist... */
      bIndexGetKey(data->loc1, public_key, &(data->public_name_1)) ;
      bIndexGetKey(data->loc2, public_key, &(data->public_name_2)) ;
    }
  ...
  if (!bsGetKeyTags(obj, _Calculation, &type))
  ...
  else
    {
      ...
      /* This is so ghastly I almost can't look at it...it relies on 4 ints in a struct
       * being contiguous..... */
      p = &data->n1;
	...
	barf = TRUE;
      ...
      if (barf)
	{
	  messerror("%s - wrong number of data (%d) for 2 point calculation %s",
		    nameWithClassDecorate(key), i, name(type));
...
BOOL getPos (MAPCONTROL map, KEY key, float *y)
{
  ...
  if (!a)
	  ...
	  && (cacheArray = arrayGet(posCache, struct cacheEntry, "kf")))
	{
	  ...
	  for (i = 2; i <arrayMax(cacheArray); i++)
	    { 
	      ...
	      key = arr(cacheArray, i,struct cacheEntry).key ;

	      assInsert(a, assVoid(key), u.vs) ;
	    }
	...
	{ 
      ...
      /* If we can't find display tags in this map, we look in any it inherits from */
      map->min = xmin;
      map->max = xmax;
      chrom = map->key;
...
int gMapGetMapObject(KEY key, KEY map, KEY hint, int ind, float *xret, float *dxret, OBJ *objp, int *count)
{
  OBJ item;	
  int flags;
  float offset;
