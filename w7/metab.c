 ...
 * Exported functions:
 * HISTORY:
 * Last edited: Mar 14 15:40 2001 (edgrif)
...
/* $Id: metab.c,v 1.25 2001-03-15 13:49:44 edgrif Exp $ */
...
{ if(!WarnSave)
    { messout("Warning: You must also save changes from main menu.");
      WarnSave = 1; }
...
static void diagQuit(void)
{ if (ActiveDiagram()->modified && messQuery("Save layout?"))
   SaveDiagram(ActiveDiagram());
...
BOOL metabDisplay (KEY key, KEY from, BOOL isOldGraph, void *unused)
{ 
...
It will not be included in the diagram.",name(rxnkey));
	     ...
	     arc = MakeArc(id++,from,to,lab = MakeText(id++,label,0,0,1,ArcColor),0);
		 ...
		 if(bsFindTag(rxn,reactant))
		   { /* Assumes only 1 reactant and product per tangent arc */
		     ...
		     from = MakeNode(id++,MakeText(id++,fromkey==0?"":name(fromkey),
						   0,0,0,CofColor),0,0);
		     to = MakeNode(id++,MakeText(id++,tokey==0?"":name(tokey),
						 0,0,0,CofColor),
				   0,0);
...
{ int j,k;
  ...
  if(!node[i]->mark)
    ...
    {			/* Possible adjacent cycle:
      ...
      int inprog = 0, containsi = 0, inprognode;
      while(c!=NULL)
...
msg("End for depth after");
  ...
  msg("Assign coords");
  { int maxelts = 0, maxeltd = 0, allknown, done, depth, i, j;
	...
	for(depth=0;depth<=maxd;depth++) 
	  if(nnodes[depth]>maxelts)
	    { maxelts = nnodes[depth]; maxeltd = depth; }
