 ...
 * HISTORY:
 * Last edited: May  6 10:50 2003 (edgrif)
...
/* $Id: pmapdisp.c,v 1.50 2003-05-06 13:13:49 edgrif Exp $ */
/*
#define ARRAY_CHECK
#define MALLOC_CHECK
*/
...
#include "aceio.h"
#include "bump.h"
#include "bump_.h"		/* provide access to internal members of BumpStruct - YUCK */
#include "lex.h"
#include "bs.h"
#include "plot.h"
#include "a.h"
...
static void pMapPick(int box, double x, double y, int modifier_unused);
...
static void mapKbd (int k, int modifier_unused) ;
...
static PhysMap currentPhysMap (char *caller)
{
 ...
 /*called by display()/display.c as a functional*/
BOOL pMapDisplay (KEY key, KEY from, BOOL isOldGraph, void *unused)
{
  ...
  case MENU_SET_ZOOM:
    {
      ...
      if ((zoom_in = messPrompt("Increase to zoom in, decrease to "
				"zoom out:",
				messprintf("%.2f",Xmagn), "fz", 0)))
	{
	  float old = Xmagn ; /* mhmp 04.12.98 pour eviter Xmagn = 0 */

	  aceInFloat (zoom_in, &Xmagn);
	  aceInDestroy (zoom_in);
	  if (!Xmagn)
	    Xmagn = old ;
	  else
...
{
  int i;
  int min = -(int)PMAP_MINUS_INFINITY, max = (int)PMAP_MINUS_INFINITY;
...
static void setDisplayDepths (void)
{
  ...
  lines_in = messPrompt ("Numbers of lines for probes, virtual "
			 "clones, fingerprinted clones:",
			 messprintf ("%d %d %d", nProbe, nYAC, nReg),
			 "iiiz", 0);
  if (lines_in)
    { 
      old = nProbe ;
      aceInInt (lines_in, &nProbe) ;
      if (nProbe <= 0)
	nProbe = old ;
      old = nYAC ;
      aceInInt (lines_in, &nYAC) ;
      if (nYAC <= 0)
	nYAC = old ;
      old = nReg ;
      aceInInt (lines_in, &nReg) ;
      if (nReg <= 0)
	nReg = old ;

      aceInDestroy (lines_in);
    }
}
...
static void pMapFollow(PhysMap look, double x, double y)
{
  ...
  if (look->activebox == gMapBox)
      ...
      else if (class(seg->key) == _VSequence)
	{
	  if (SegLen(seg)!=0)
#ifdef USE_SMAP
	    from=KEYMAKE(_VCalcul, sMapLength(seg->key)*x/(20.0*Xmagn*(0.5*SegLen(seg))/*ie old dx*/));
#else
...
void pMapDrawGIF (ACEIN command_in, ACEOUT result_out)
{
  KEY key, from = 0 ;
  int x ;
  int z1, z2, winWidth ;
  ...
  while (aceInStep (command_in, '-'))	/* options */
    if ((word = aceInWord(command_in)))
      {
	if (strcmp (word, "clone") == 0
	    && aceInCheck (command_in, "w"))
	  {
	    word = aceInWord(command_in);
	    if (!lexword2key (word, &key, _VClone))
	      {
		aceOutPrint (result_out, "// gif pmap error: clone %s not known\n", word) ;
		return ;
	      }
	  }
	else if (strcmp (word, "contig") == 0
		 && aceInCheck (command_in, "wi"))
	  {
	    word = aceInWord(command_in);
	    if (!lexword2key (word, &key, _VContig))
	      {
		aceOutPrint (result_out, "// gif pmap error: contig %s not known\n", word) ;
		return ;
	      }
	    aceInInt (command_in, &x) ;
	    x |= 0x800000 ;
  ...
  if (class(key) == _VClone)
    { OBJ obj = bsUpdate (key) ;
      if (!obj)
	{
	  aceOutPrint (result_out, "// gif pmap error: clone %s has no data\n", freeprotect(name(key))) ;
	  return ;
	}
      if (!bsGetKey (obj, _pMap, &key))	/* replace key with contig */
	{
	  aceOutPrint (result_out, "// gif pmap error: clone %s has no pMap\n", freeprotect(name(key))) ;
	  bsDestroy (obj) ;
	  return ;
	}
      bsDestroy (obj) ;
  ...
  aceOutPrint (result_out, "// PMAP %s %d %d\n", freeprotect(name(key)), z1, z2) ;

  return ;

usage:
  aceOutPrint (result_out, "// gif pmap error: usage: PMAP [-clone <name>]|[-contig <name> <coord>]\n") ;
