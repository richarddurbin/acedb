...
static BOOL parseHtml (char **cp, HtmlNode **resultnode, STORE_HANDLE handle)
{
  ...
  *cp += 6 ;			/* skip <HTML> */
  ...
  if (!(parseSection (cp, &leftnode, handle)))
    {
      fprintf (stderr, "Help warning : text inside <HTML> not valid !!\n") ;
    }
  ...
  if (strncasecmp (*cp, "</HTML>", 7) == 0)
    ...
    {
      fprintf (stderr, "Help warning : <HTML> tag not closed by </HTML> !!\n") ;
    }
...
static BOOL parseHead (char **cp, HtmlNode **resultnode, STORE_HANDLE handle)
{
  ...
  *cp += 6 ;			/* skip <HEAD> */
  ...
  if (!(parseSection (cp, &leftnode, handle)))
    {
      fprintf (stderr, "Help warning : HTML inside <head> not valid !!\n") ;
    }
  ...
  if (strncasecmp (*cp, "</HEAD>", 7) == 0)
    ...
    {
      fprintf (stderr, "Help warning : <HEAD> tag not closed by </HEAD> !!\n") ;
    }
...
static BOOL parseBody (char **cp, HtmlNode **resultnode, STORE_HANDLE handle)
{
  ...
  *cp += 1 ;			/* skip '>' */
  ...
  if (!(parseSection (cp, &leftnode, handle)))
    {
      fprintf (stderr, "Help warning : HTML inside <BODY> not valid !!\n") ;
    }
  ...
  if (strncasecmp (*cp, "</BODY>", 7) == 0)
    ...
    {
      fprintf (stderr, "Help warning : <BODY> tag not closed by </BODY> !!\n") ;
    }
...
static BOOL parseHeader (char **cp, int level, HtmlNode **resultnode, STORE_HANDLE handle)
{
  HtmlNode *node, *leftnode ;
  ...
  *cp += 1;			/* skip '>' */
  ...
  if (!(parseSection (cp, &leftnode, handle)))
    {
      fprintf (stderr, "Help warning : heading%d text not valid !!\n", level) ;
    }
  ...
  if ((strncasecmp (*cp, "</H", 3) == 0) &&
    ...
    {
      fprintf (stderr, "Help warning : <H%d> tag not closed by </H%d> !!\n", level, level) ;
    }
...
static BOOL parseCode (char **cp, HtmlNode **resultnode, STORE_HANDLE handle)
{
  ...
  *cp += 6 ;			/* skip <CODE> */
  ...
  if (!(parseSection (cp, &leftnode, handle)))
    {
      fprintf (stderr, "Help warning : <code> text not valid !!\n") ;
    }
  ...
  if (strncasecmp (*cp, "</CODE>", 7) == 0)
    ...
    {
      fprintf (stderr, "Help warning : <CODE> tag not closed by </CODE> !!\n") ;
    }
...
static BOOL parseBold (char **cp, HtmlNode **resultnode, STORE_HANDLE handle)
{
  ...
  *cp += 3 ;			/* skip <B> */
  ...
  if (!(parseSection (cp, &leftnode, handle)))
    {
      fprintf (stderr, "Help warning : HTML inside <B> not valid !!\n") ;
    }
  ...
  if (strncasecmp (*cp, "</B>", 3) == 0)
    ...
    {
      fprintf (stderr, "Help warning : <B> tag not closed by </B> !!\n") ;
    }
...
static BOOL parseUnderlined (char **cp, HtmlNode **resultnode, STORE_HANDLE handle)
{
  HtmlNode *node, *leftnode ;
  ...
  *cp += 3 ;			/* skip <B> */
  ...
  node = makeNode (HTML_UNDERLINED_STYLE, handle) ;

  if (!(parseSection (cp, &leftnode, handle)))
    {
      fprintf (stderr, "Help warning : HTML inside <U> not valid !!\n") ;
    }

  skipSpaces (cp) ;

  if (strncasecmp (*cp, "</U>", 3) == 0)
    {
      *cp += 4 ;
    }
  else
    {
      fprintf (stderr, "Help warning : <U> tag not closed by </U> !!\n") ;
    }
...
static BOOL parseStrong (char **cp, HtmlNode **resultnode, STORE_HANDLE handle)
{
  ...
  *cp += 8 ;			/* skip <STRONG> */
  ...
  if (!(parseSection (cp, &leftnode, handle)))
    {
      fprintf (stderr, "Help warning : strong text not valid !!\n") ;
    }
  ...
  if (strncasecmp (*cp, "</STRONG>", 9) == 0)
    ...
    {
      fprintf (stderr, "Help warning : <STRONG> tag not closed by </STRONG> !!\n") ;
    }
...
static BOOL parseItalic (char **cp, HtmlNode **resultnode, STORE_HANDLE handle)
{
  ...
  *cp += 3 ;			/* skip <I> */
  ...
  if (!(parseSection (cp, &leftnode, handle)))
    {
      fprintf (stderr, "Help warning : bold text not valid !!\n") ;
    }
  ...
  if (strncasecmp (*cp, "</I>", 3) == 0)
    ...
    {
      fprintf (stderr, "Help warning : <I> tag not closed by </I> !!\n") ;
    }
...
static BOOL parseHref (char **cp, HtmlNode **resultnode, STORE_HANDLE handle)
{
  ...
  *cp += 2 ;			/* skip '<A' */
  ...
  if (strncasecmp (*cp, "HREF=", 5) == 0)
    ...
    {
      fprintf (stderr, "Help warning : anchor tag <A without argument !!\n");
      HAVE_HREF = FALSE ;
    }
  ...
  if (!(parseSection (cp, &leftnode, handle)))
    {
      fprintf (stderr, "Help warning : referenced text not valid !!\n") ;
    }
  ...
  if (strncasecmp (*cp, "</a>", 4) == 0)
    ...
    {
      fprintf (stderr, "Help warning : anchor tag not closed by </A> !!\n") ;
    }
...
static BOOL parseList (int style, char **cp, HtmlNode **resultnode, STORE_HANDLE handle)
{
  ...
  *cp += 4 ;			/* skip <UL> */
  ...
  if ((style == HTML_LIST_BULLET && strncasecmp (*cp, "</ul>", 5) == 0) ||
	...
	fprintf (stderr, "Help warning : found <UL> without closing </UL> tag !!\n") ;
      else if (style == HTML_LIST_NOINDENT)
	fprintf (stderr, "Help warning : found <DL> without closing </DL> tag !!\n") ;
      else if (style == HTML_LIST_NUMBER)
	fprintf (stderr, "Help warning : found <OL> without closing </OL> tag !!\n") ;
    }
...
static BOOL parseSection (char **cp, HtmlNode **resultnode, STORE_HANDLE handle)
{
  ...
  if (!**cp)			/* EOF */
    {
      if (MODE_PREFORMAT)
	fprintf (stderr, "Help warning : found <PRE> tag "
		"without closing </PRE> tag !!\n") ;
      if (MODE_BLOCKQUOTE)
	fprintf (stderr, "Help warning : found <BLOCKQUOTE> tag "
  ...
  if (strncasecmp (*cp, "<!--", 4) == 0)
  ...
  else if (strncasecmp (*cp, "<body", 5) == 0)
    {
	   ...
	   (*cp)[2]-'0' >= 1 && (*cp)[2]-'0' <= 7)
    {
      ...
      ++(*cp) ;++(*cp) ;
      while (**cp && **cp != '>') { ++(*cp) ; }
      if (!parseHeader (cp, level, &leftnode, handle))
	{
	  *resultnode = 0 ;
	  return FALSE ;
	}
    ...
    {
      if (MODE_PREFORMAT)
	fprintf (stderr, "Help warning : nesting of <PRE> tags without effect !!\n") ;
      MODE_PREFORMAT = TRUE ;
	...
	fprintf (stderr, "Help warning : nesting of <BLOCKQUOTE> tags "
		"without effect !!\n") ;
  ...
  else if (strncasecmp (*cp, "<u>", 3) == 0)
    {
      if (!(parseUnderlined (cp, &leftnode, handle)))
	{
	  *resultnode = 0 ;
	  return FALSE ;
	}
    }
  else if (strncasecmp (*cp, "</u>", 4) == 0)
    {
      *resultnode = 0 ;
      return TRUE ;
    }
  ...
  if (leftnode->type == 0)
    {
      printf ("Help warning : section on section \n") ;
    }
