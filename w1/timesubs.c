 ...
 * * i moved the non thread safe functions to freesubs,
...
/* $Id: timesubs.c,v 1.42 2002-02-22 18:00:21 srk Exp $ */
		...
		BOOL *wantMins, BOOL *wantSecs)
{
  ...
  { 
    ...
    mktime(tm); 
#ifdef SGI
    /* mktime is bugged on some SGIs, it adjusts between summer and winter
       time even though we set tm_isdst to be negative. 
       We detect when this has happened, and use the old (pre mktime call)
       values for tm instead, but get values from the mktime
       call for wday and yday. We have to adjust these for day rollover */
    if (tm->tm_hour != save.tm_hour)
      {
	save.tm_wday = tm->tm_wday;
	save.tm_yday = tm->tm_yday;
	if ((tm->tm_hour == 0) && (save.tm_hour == 23))
	  { /*  bug rolled forward a day move it back*/
	    save.tm_wday--;
	    if (save.tm_wday == -1) save.tm_wday = 6;
	    save.tm_yday--;
	    if (save.tm_yday == -1) save.tm_yday = 364;
	  }
	else if ((tm->tm_hour == 23) && (save.tm_hour == 0))
	  { /* bug rolled it back over a day move it forward */
	    save.tm_wday++;
	    if (save.tm_wday == 7) save.tm_wday = 0;
	    save.tm_yday++;
	    if (save.tm_yday == 365) save.tm_yday = 0;
	  }
...
/**********************************************/
/* thread safe, ace_time must be 25 char long */
char *timeShowJava2 (char* ace_time, mytime_t t) 
{
  struct tm ts;
  ...
  if (!wantMonth)
    ...
    strftime (ace_time, 25, "%d %b %Y %H:%M:00", &ts);
  else
    strftime (ace_time, 25, "%d %b %Y %H:%M:%S", &ts) ;
...
char *timeShow2 (char *ace_time, mytime_t t) 
{
  struct tm ts;
  BOOL wantMonth, wantDay, wantHours, wantMins, wantSecs;
  ...
  if (!wantMonth)
    ...
    strftime (ace_time, 25, "%Y-%m-%d_%H:%M", &ts);
  else
    strftime (ace_time, 25, "%Y-%m-%d_%H:%M:%S", &ts) ;
...
/*************************************************************/
/* thread safe, buf  must be 25 char long */

char *timeDiffShow2 (char *buf, mytime_t t1, mytime_t t2) 
{
  struct tm ts1, ts2;
  BOOL wantMonth1, wantDay1, wantHours1, wantMins1, wantSecs1;
  BOOL wantMonth2, wantDay2, wantHours2, wantMins2, wantSecs2;
