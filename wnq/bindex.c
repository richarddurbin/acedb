 ...
 * HISTORY:
 * Last edited: Jul 11 08:55 2003 (edgrif)
 * Created: August 1 1997, jtm/rd
 * CVS info:   $Id: bindex.c,v 1.61 2004-04-22 22:25:26 mieg Exp $
 *-------------------------------------------------------------------
 */
...
#ifdef ACEDB5
/* note that current index version has nothing to do with
 acedb main release number, they just happen to coincide
 as of oct 98
 */
...
int bIndexTagCount (int cc, KEY tag)
{
  int result = 0 ;
  int tt ;
  Array a ;
...
static void bIndexDatabase (void)
{
  ...
  messout("Indexed %d tags in %d objects totalling %d nodes \n", 
...
void bIndexNewModels(void)
{
  ...
  messout("Reindexed %d tags in %d classes containing %d objects totalling %d nodes \n", 
	  lastTt, nIndexedClasses, nIndexedObjs, nIndexedNodes) ;
  messdump("Indexed %d tags in %d classes containing %d objects "
	   "totalling %d nodes", 
	   lastTt, nIndexedClasses, nIndexedObjs, nIndexedNodes) ;
