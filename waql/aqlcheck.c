 ...
 *
 * SCCS: $Id: aqlcheck.c,v 1.61 2004-10-08 00:26:24 mieg Exp $
 * Description:
 * Exported functions:
 * HISTORY:
 ...
 * * Jul 16 17:17 1998 (fw): STORE_HANDLE made global and renamed globalStoreHandle
...
AqlQuery *aqlQueryCreate (const char *queryText, STORE_HANDLE objectHandle)
/*
...
*/
{
  ...
  while (*cp)
      ...
      while (*cp)
	{
      ...
      if (!*cp)			/* got to the end of the string */
	break;

      /* found a whitespace */
      cws = cp;			/* remember that position */

      cp++;			/* skip this space and move to the next position, 
				   that might also be a white-space */

      /* find the next non-white-space */
      while (*cp)
	{
	  if (!(*cp == ' ' || *cp == '\t' || *cp == '\n' || *cp == '\r' || *cp == '\f'))
	    break;
...
AqlScope* aqlScopeCreate (STORE_HANDLE handle)	/* also used in aql.c */
{
...
static AqlNode *makeNewNode (AqlNodeType type, STORE_HANDLE handle)
{ 
                                   ...
                                   AQL aql)
{
  ...
  if (aql->parseDebugLevel >= 3)
    {
      aceOutPrint(aql->debug_out, "make artificial FROM_clause declaration, Label %s No %d - ",
                  label,
...
/* Now start on remapping the declarations.  First, maintain our pointers. */
  ...
  if (inNode->type == nTABLE_SFW)		
    { 
      ...
      aql->checkEnv->currFromLoc = 0 ;
#ifdef DEBUG
      aceOutPrint(aql->debug_out, "Setting currdecl\n");
  ...
  */
  ...
  if (aql->checkEnv->currFromLoc && 
    ...
    { 
      ...
      aql->checkEnv->currFromLoc = 0 ;
#ifdef DEBUG
      aceOutPrint(aql->debug_out, "Changing currdecl\n");
      if (aql->checkEnv->currFromLoc == NULL)
      {
                aceOutPrint(aql->debug_out, "but nothing there\n");
      } else
      {
...
/* now do the work itself */
  ...
  if (inNode->type == nLOC_LOCAL_POS   || inNode->type == nLOC_LOCAL_TAG_NAME  ||
	...
	{				/* transfer parent to newNode->nxt */
      ...
      if (aql->parseDebugLevel >= 2)
      {
          aceOutPrint(aql->debug_out, "Making transferparent for:\n");
...
/* shouldn't we start that at NULL, to be on the safe side?
   ...
   sure to? */
        ...
        if (aql->parseDebugLevel >= 4) {
            aceOutPrint(aql->debug_out, "Looking in scopes for locator matching name \"%s\" to suit node:\n", reference_name);
            ...
            aceOutPrint(aql->debug_out, "The name from the inner node is \"%s\" and the name from the outer node is \"%s\" and the reference name is \"%s\"\n",
                        naming_inner_node ? naming_inner_node->name : NULL, naming_outer_node->name, reference_name);

            {
                ...
                {
                    aceOutPrint(aql->debug_out, "decl %d:\n", decl_i);
                    if (pdecl && *pdecl)
                    {
...
*/
        ...
        {
            for (scopeDictIndex = 0 ; scopeDictIndex < dictMax(scope->dict) ; scopeDictIndex++)
            {
	      if (array(scope->loc, scopeDictIndex, AqlLoc*)) /* mieg: arr->array, i crahsed oct 8 2004 */
                {
...
*/
            ...
            && !(locatorDefinitionNode->loc->source == IS_ROW))
        {
            ...
            if (aql->parseDebugLevel >= 4) {
                aceOutPrint(aql->debug_out, "Found existing loc %lx\n", locatorDefinitionNode->loc);
                aceOutPrint(aql->debug_out, "Assigning the locator already defined at node:\n");
                aqlNodeOut(aql->debug_out, locatorDefinitionNode, level, "existingloc", TRUE);
                aceOutPrint(aql->debug_out, "To be also the locator of node:\n");
        ...
        { 
            ...
            array(aql->checkEnv->currentLocScope->loc, varNameDictIndex, AqlLoc*) = inNode->loc;
            if (aql->parseDebugLevel >= 4) {
                aceOutPrint(aql->debug_out, "Did not find existing loc, making new one at %lx for node:\n", inNode->loc);
        ...
        for (selectFieldNode = inNode->right, col = 0 ; selectFieldNode ; selectFieldNode = selectFieldNode->nxt, ++col)
            if (selectFieldNode->name)
            {
                if (selectFieldNode->name[0] == '_') /* temp. name */
                    tableSetColumnName (inNode->value.T, col, "") ;
                else
