/* $Id: aqlrun.c,v 1.104 2004-09-02 15:30:23 edgrif Exp $ */
/*  File: aqlrun.c
   ...
   returns 0, if the return value is of a different type */
{
 ...
 rerun:
  ...
  if (aql->evalDebugLevel >= 1)
    aceOutPrint(aql->debug_out, "Returning table of %d entries\n", tableMax(resultNode->value.T));
                                       ...
                                       int level)
{
  ...
  switch (inNode->type)
	...
	if (!lexword2key (inNode->name, &inNode->value.k, 0))
	  aqlError (aql, 944, inNode->pos, "Unrecognized tag %s", inNode->name) ;
	...
	if (lexword2key (inNode->name, &key, _VClass))
	  ...
	  aqlError (aql, 941, inNode->pos, "Unrecognized class %s", inNode->name) ;
      }
      break;
	...
	if (inNode->left->type == nTEXT ||
	    (inNode->left->loc && inNode->left->loc->type == 's'&& inNode->left->loc->isValue == KNOWNVAL)) 
	  {
		...
		inNode->left->isValue = KNOWNVAL;
	      ...
	      aqlError (aql, 942, inNode->pos, "Unrecognized class %s in construct object(\"<class>\", ...)",
	...
	if (fromLocNode->left->type == nCLASS)
	...
	else if (fromLocNode->left->nxt)
	  {
		...
		*/
		aqlError (aql, 930, fromLocNode->left->nxt->pos, "Invalid column-selection on %s:%d\n"
			  "The %s is not row-based.", 
			  (strcmp(defNode->name, "_DEF") == 0) ? "" : defNode->name,
			  fromLocNode->left->nxt->number,
			  (strcmp(defNode->name, "_DEF") == 0) ? "default iterator" : messprintf("variable %s", defNode->name)) ;
	      }
		...
		*/
		aqlError (aql, 931, fromLocNode->left->nxt->pos, "Invalid column-selection on %s:%s\n"
			  "The %s is not row-based.", 
			  (strcmp(defNode->name, "_DEF") == 0) ? "" : defNode->name,
	      ...
	      aqlError (aql, 620, 0,
			"postAssignTypesAndClasses() - invalid type of declaration for %s",
			inNode->loc->definition->left->name);
	  }
	else
	  aqlError (aql, 620, 0,
		    "postAssignTypesAndClasses() - invalid type (%s) of fromLocNode->left", 
	...
	for (selectNodeLeft = inNode->left->right, selectNodeRight = inNode->right->right, col = 0; 
	     ...
	     selectNodeLeft = selectNodeLeft->nxt, selectNodeRight = selectNodeRight->nxt, ++col)
	  {
	    ...
	    if (leftValueType != rightValueType)
	      ...
	      aqlError (aql, 960, inNode->pos, "table combining operator - error in column %d\n"
                   ...
                   int      level)
{
    ...
    if (!consumer->sorting)
    {
        aceOutPrint(aql->debug_out, "%x->%x not sorting, should output table header here? or a bit later...\n", tableNode, consumer);
    }
                                ...
                                int     level)
{
        ...
        else
        {
            ...
            if (!definitionLocator->trueObj)
            {
                if (aql->evalDebugLevel >= 4) {
                    indent(aql, level);
                    aceOutPrint (aql->debug_out, "wakeUpLocal : about to bsCreate(%x)\n",
            ...
            if (aql->evalDebugLevel >= 4) {
                indent(aql, level);
                aceOutPrint (aql->debug_out, "wakeUpLocal : locator %lx created and marked(%s) useObj = %lx (from trueObj of loc=%lx)\n",
        ...
        /* we already have the object to use (will never happen in fuzzy run) */
    {
        ...
        if (aql->evalDebugLevel >= 4) {
            indent(aql, level);
            aceOutPrint (aql->debug_out, "wakeUpLocal : locator %lx jumped to mark(%s) in useObj = %lx\n",
                         (unsigned long)inLocator,
                         bsGetMarkAsString(inLocator->mark),
                  ...
                  int     level)
{
  if (!inLocator->trueObj)	
	...
	return FUZZYVAL;
      ...
      if (aql->evalDebugLevel >= 4) {
          indent(aql, level);
          aceOutPrint (aql->debug_out, "wakeUpFollow : about to bsCreate(%x)\n",
                       inLocator->value.k);
      }
	...
	return NULLVAL ;
      ...
      if (aql->evalDebugLevel >= 4) {
          indent(aql, level);
          aceOutPrint (aql->debug_out, "wakeUpFollow : locator %lx created and marked(%s) trueObj = %lx\n",
     ...
     we'll never have an object in fuzzy mode */
    {
      ...
      if (aql->evalDebugLevel >= 4) {
          indent(aql, level);
          aceOutPrint (aql->debug_out, "wakeUpFollow : locator %lx jumped to mark(%s) in trueObj = %lx\n", 
                       (unsigned long)inLocator, bsGetMarkAsString(inLocator->mark), (unsigned long)inLocator->trueObj);
      }
    }
 ...
 * it then traverses through the FROM_LOCs in processThisLoc, which calls it on the ->nxt's*/
{
    ...
    if (aql->evalDebugLevel >= 3) {
        indent(aql, level); aceOutPrint (aql->debug_out, "loopVariables (%s):\n", isFuzzyRun ? "fuzzy" : "definite");
        if (aql->evalDebugLevel >= 3) {
    ...
    { 
	  ...
	  aqlError (aql, 620, 0,"loopVariables():IS_COLUMN - \n"
        ...
        if (aql->evalDebugLevel >= 4) {
            indent(aql, level); aceOutPrint (aql->debug_out, "eval IS_COLUMN in Loc %lx\n", (unsigned long)fromFieldNode->loc) ;
        }
    ...
    case IS_METHOD:
    {
        ...
        if (aql->evalDebugLevel >= 4) {
            indent(aql, level); aceOutPrint (aql->debug_out, "eval IS_METHOD in Loc %lx\n", (unsigned long)fromFieldNode->loc) ;
        }
                 ...
                 strcasecmp(methodNode->nxt->name, "update_session") == 0)
        {
            ...
            switch (freelower(methodNode->nxt->name[0]) + freelower(methodNode->nxt->name[7]))
            {
                 ...
                 strcasecmp(methodNode->nxt->name, "node_time") == 0)
        {
                ...
                methodNode->loc->source == IS_FOLLOW_POS)
            {
                ...
                switch (freelower(methodNode->nxt->name[0]) + freelower(methodNode->nxt->name[5]))
                {
                case 'n'+'s':
                case 't'+'t':
            ...
            aqlError (aql, 703, methodNode->nxt->pos+1, "syntax error : unrecognised Method-name '%s'", methodNode->nxt->name);
    }
    ...
    { 
        ...
        if (aql->evalDebugLevel >= 4) {
            indent(aql, level); aceOutPrint (aql->debug_out, "eval IS_OBJECT in Loc %lx\n", (unsigned long)fromFieldNode->loc) ;
        }
        ...
        if (aql->evalDebugLevel >= 4) {
            indent(aql, level); aceOutPrint (aql->debug_out, "left     expr value=%s (key=%x)\n",
        ...
        if (aql->evalDebugLevel >= 4) {
            indent(aql, level); aceOutPrint (aql->debug_out, "right    expr value=%s (key=%x)\n",
                                             name(objNode->right->value.k), objNode->right->value.k);
        }

        if (aql->evalDebugLevel >= 4) {
            indent(aql, level); aceOutPrint (aql->debug_out, "eval done loc_value=%s (key=%x)\n",
    ...
    { 
        ...
        if (aql->evalDebugLevel >= 4) {
            ...
            indent(aql, level); aceOutPrint (aql->debug_out, " - FROM_LOC: %s", fromFieldNode->name);
            aqlNodeOut (aql->debug_out, fromFieldNode, level, "follow from", TRUE) ;
#if 0
            aceOutPrint (aql->debug_out, "eval IS_FOLLOW_TAG in Loc %lx - FROM_LOC %s (%lx)\n",
                         (unsigned long)fromFieldNode->loc,
                         fromFieldNode->name, 
        ...
        if (aql->evalDebugLevel >= 4) {
            indent(aql, level); aceOutPrint (aql->debug_out, "followtag looking for tag %x (\"%s\") in object %x (\"%s\") -> (%s)\n",
        ...
        case 0:		/* tag doesn't exist */
            ...
            if (aql->evalDebugLevel >= 4) {
                indent(aql, level); aceOutPrint (aql->debug_out, "eval done by follow: tag absent\n");
            }
        ...
        case 1:		/* tag might exist - open object now and check */
            ...
            {
                if (aql->evalDebugLevel >= 4) {
                    indent(aql, level); aceOutPrint (aql->debug_out, "eval done : fuzzy\n");
                }
            ...
            else
            {
                if (!definitionLocator->trueObj)
                {
                    ...
                    if (aql->evalDebugLevel >= 4) {
                        indent(aql, level);
                        aceOutPrint(aql->debug_out, "IS_FOLLOW_TAG opening object: definition_locator %lx created trueObj %lx (\"%s\")\n",
                ...
                if (aql->evalDebugLevel >= 4) {
                    indent(aql, level); aceOutPrint(aql->debug_out, "eval done opening object: from_locator %lx got useObj = %lx and marked(%s)\n",
        ...
        case 2:		/* tag does exist */
            ...
            if (aql->evalDebugLevel >= 4) {
                indent(aql, level); aceOutPrint(aql->debug_out, "eval done : from_locator %lx zeroes useObj\n",
                                                (unsigned long)fromFieldNode->loc);
            }
    ...
    { 
        ...
        if (aql->evalDebugLevel >= 4) {
            indent(aql, level);
            aceOutPrint (aql->debug_out, "eval IS_LOCAL_TAG in Loc %lx\n", (unsigned long)fromFieldNode->loc) ;
        }
        ...
        else
        {
            ...
            if (aql->evalDebugLevel >= 4) {
                indent(aql, level);
                aceOutPrint (aql->debug_out, "loc=%lx - moved to tag %s in useObj = %lx and marked(%s)\n",
                             (unsigned long)fromFieldNode->loc,
         ...
         * has the value of the TABLE_SFW node that created the table */
    {
        ...
        if (aql->evalDebugLevel >= 4) {
            indent(aql, level); aceOutPrint(aql->debug_out, "eval IS_ROW in Loc %lx (column %d)\n",
        ...
        if (aql->evalDebugLevel >= 4) {
            indent(aql, level); aceOutPrint(aql->debug_out, "zero row-counter for column %d\n",
                                            fromLocNode->loc->column);
        }
    ...
    { 
        ...
        if (aql->evalDebugLevel >= 4) {
            indent(aql, level); aceOutPrint(aql->debug_out, "eval IS_(LOCAL/FOLLOW)_POS in Loc %lx\n", (unsigned long)fromFieldNode->loc) ;
            indent(aql, level); aceOutPrint(aql->debug_out, "locname is %s\n", fromFieldNode->loc->definition->name);
        }
        ...
        if (fromFieldNode->loc->source == IS_LOCAL_POS)
        {
            ...
            if (aql->evalDebugLevel >= 4) {
                indent(aql, level); aceOutPrint (aql->debug_out, "local/follow_pos waking up local\n") ;
            }
        ...
        if (aql->evalDebugLevel >= 4) {
            indent(aql, level); aceOutPrint(aql->debug_out, "fromFieldNode->loc->key is %s(%x)\n",
        ...
        { /* we already have a mark and need to save it
           * before we start iterating over the multivalues in this loc */
            ...
            if (aql->evalDebugLevel >= 4) {
                indent(aql, level); aceOutPrint(aql->debug_out, "loc = %lx - pushed existing mark(%s) in useObj\n",
                                                (unsigned long)fromFieldNode->loc,
                                                bsGetMarkAsString(fromFieldNode->loc->mark),
                                                (unsigned long)fromFieldNode->loc->useObj);
            }
        ...
        if (aql->evalDebugLevel >= 4) {
            indent(aql, level); aceOutPrint(aql->debug_out, "eval done : from_locator %lx gets value from useObj = %lx and marked(%s)\n",
                                            (unsigned long)fromFieldNode->loc, (unsigned long)fromFieldNode->loc->useObj,
                                            bsGetMarkAsString(fromFieldNode->loc->mark));
        }
    ...
    default:
      aqlError (aql, 620, 0,
		"loopVariables() - invalid fromFieldNode->loc->source %s",
    ...
    while(TRUE)			/* escape only by return in iteration step, no breaks */
    {
        ...
        if (aql->evalDebugLevel >= 3) {
            indent(aql, level); aceOutPrint(aql->debug_out, "multi value on iteration %d", counter);
        ...
        case IS_CLASS:
        {
            ...
            {
                if (aql->evalDebugLevel >= 3) {
                    indent(aql, level); aceOutPrint (aql->debug_out, "end CLASS multi value\n");
                }
                return status ;
            }
        }
        ...
        case IS_ROW:
        {
            ...
            if (aql->evalDebugLevel >= 4) {
                indent(aql, level); aceOutPrint(aql->debug_out, "next ROW in loc=%lx column %d - current at row %d\n",
                                                (unsigned long)fromTableNode->loc,
                                                fromLocNode->loc->column,
            ...
            {
                if (aql->evalDebugLevel >= 3) {
                    indent(aql, level); aceOutPrint(aql->debug_out, "end ROW multi value in column %d\n",
                                                    fromLocNode->loc->column);
                }
        ...
        case IS_FOLLOW_POS:
            if (aql->evalDebugLevel >= 4) {
                indent(aql, level); aceOutPrint(aql->debug_out, "next POS in loc=%lx - jumped to mark(%s) useObj = %lx\n",
                                                (unsigned long)fromFieldNode->loc,
                                                bsGetMarkAsString(fromFieldNode->loc->mark),
                                                (unsigned long)fromFieldNode->loc->useObj);
            }
            ...
            if (!bsGetKeyTags (fromFieldNode->loc->useObj, _bsDown, 0))
            {
                ...
                if (aql->evalDebugLevel >= 3) {
                    indent(aql, level); aceOutPrint (aql->debug_out, "end (LOCAL/FOLLOW)_POS multi value\n");
                }
                ...
                if (stackExists (fromFieldNode->loc->markStack))
                {
                    ...
                    if (aql->evalDebugLevel >= 4) {
                        indent(aql, level); aceOutPrint(aql->debug_out, "loc=%lx - popped markStack useObj = %lx and jumped to mark(%s)\n",
                                                        (unsigned long)fromFieldNode->loc,
                                                        (unsigned long)fromFieldNode->loc->useObj,
                                                        bsGetMarkAsString(fromFieldNode->loc->mark));
                    }
            ...
            if (aql->evalDebugLevel >= 4) {
                indent(aql, level); aceOutPrint(aql->debug_out, "loc=%lx - moved down useObj = %lx and marked(%s)\n",
                                                (unsigned long)fromFieldNode->loc, (unsigned long)fromFieldNode->loc->useObj,
                                                bsGetMarkAsString(fromFieldNode->loc->mark));
            }
        ...
        default:
	  aqlError (aql, 620, 0,
		    "loopVariables() : should not loop on source type %d",
                                        ...
                                        AqlFuzzyType status)
{
    ...
    if (isFuzzyStatus) {
        if (aql->evalDebugLevel >= 4) {
            indent(aql, level); aceOutPrint (aql->debug_out, "eval fuzzy\n");
        }
        ...
        if (isEmptyStatus) {
            if (aql->evalDebugLevel >= 4) {
                indent(aql, level); aceOutPrint (aql->debug_out, "eval empty\n");
            }
            fromFieldNode->loc->isValue = NULLVAL; 
        } else {
            if (fromFieldNode->loc->isValue == KNOWNVAL &&
                aql->evalDebugLevel >= 3) {
                indent(aql, level); aceOutPrint (aql->debug_out, "single value:\n");
            }
        }
    }
    ...
    else
    {
        ...
        */
        ...
        {
            if (aql->evalDebugLevel >= 3) {
                indent(aql, level); aceOutPrint(aql->debug_out,
                                   ...
                                   int      level)
{
  ...
  if (condition)
  {
      ...
      if (aql->evalDebugLevel >= 4) {
          indent(aql, level); aceOutPrint(aql->debug_out, "where status is %s and result is %s\n",
  ...
  {
      if (aql->evalDebugLevel >= 4) {
          indent(aql, level); aceOutPrint(aql->debug_out, "either, condition is true, or status fuzzy: condition %s; status  %s\n",
                      condition ? "true" : "false",
                      status == FUZZYVAL ? "fuzzy" :
                      status == NULLVAL ? "null" :
                      "known");
      ...
      if (fromFieldNode->nxt) {
          ...
          if (aql->evalDebugLevel >= 4) {
              indent(aql, level); aceOutPrint(aql->debug_out, "There are deeper variables; processing the next one\n");
          }
      ...
      {
          if (aql->evalDebugLevel >= 4) {
              indent(aql, level); aceOutPrint(aql->debug_out, "There are no deeper variables;%s\n",
  ...
  if (fromFieldNode->loc->trueObj) /* never true in fuzzy mode */
    {
        if (aql->evalDebugLevel >= 4) {
            indent(aql, level); aceOutPrint(aql->debug_out, "processLoc: from_locator %lx lost trueObj = %lx\n",
...
/* XXXXXXXX rd says: inefficient - should only recalc when necessary! XXXXXXXX */
{
    ...
    if (aql->evalDebugLevel >= 3) {
        indent(aql, level);
        aceOutPrint (aql->debug_out, "report to results-%s:\n",
    ...
    if (rowStatus == NULLVAL)
    {
        ...
        if (aql->evalDebugLevel >= 3) {
            indent(aql, level);
            aceOutPrint (aql->debug_out, "(not adding row - all fields NULL)\n") ;
        }
    }
    else if (rowStatus == FUZZYVAL)
    {
        if (aql->evalDebugLevel >= 3) {
            indent(aql, level);
            aceOutPrint (aql->debug_out, "(not adding row - some fields fuzzy)\n") ;
        }
    }
    else if (rowStatus == KNOWNVAL)
    {
        ...
        if (sfw->resultConsumer->sorting)
        {
            if (aql->evalDebugLevel >= 3) {
                indent(aql, level);
                aceOutPrint(aql->debug_out, "outputting via table for %x, using all rows\n", sfw);
            }
            newRowNumber = tableMax(theSfwTable);	/* add at the end of table */
        } else {
            if (aql->evalDebugLevel >= 3) {
                indent(aql, level);
                aceOutPrint(aql->debug_out, "outputting directly for %x, only ever using one row\n", sfw);
            }
                ...
                /* copy the value to the table-field */
            {
			  ...
			  sfw->resultConsumer->resultTableDict = dictHandleCreate(100, aql->query->handle);
                ...
                else if (selectFieldNode->vtype != theSfwTable->type[selectFieldCounter])
                {
                    if (aql->evalDebugLevel >= 3) {
                        indent(aql, level);
                        aceOutPrint (aql->debug_out, "(type mismatch in column %d - value = NULL)",
                    ...
                    */
                    ...
                    aqlError (aql, 810, selectFieldNode->pos, "Inconsistent value types in table column %d\n"
                ...
                default:
		  aqlError (aql, 620, 0,
			    "sfwReport() - type %c not supported for table expr",
        ...
        if (isRowFound)
        {
            if (aql->evalDebugLevel >= 3) {
                indent(aql, level);
                aceOutPrint (aql->debug_out, "(not adding row - exists already)\n") ;
            }
            if (sfw->resultConsumer->sorting)
            {
        ...
        else
        {
            ...
            if (aql->evalDebugLevel >= 3) {
                indent(aql, level);
                aceOutPrint (aql->debug_out, "(adding row at line %d (%d in consumer))\n", newRowNumber, sfw->resultConsumer->nRows) ;
            }
      ...
      */
{
  ...
  switch (exprNode->type)
	...
	*/
	...
	if (exprNode->op == oUMINUS)
	  {
		...
		if (!(exprNode->left->vtype == 'i' || exprNode->left->vtype == 'f'))
		  aqlError (aql, 898, exprNode->left->pos, "incorrect expression type (%c) for unary MINUS operator", exprNode->left->vtype);
	...
	else if (exprNode->op == oABS)
	  {
		...
		if (!(exprNode->left->vtype == 'i' || exprNode->left->vtype == 'f'))
		  aqlError (aql, 899, exprNode->left->pos, "incorrect expression type (%c) for abs() ExprFunc", exprNode->left->vtype);
		...
		if (exprNode->op == oPLUS ||
		    ...
		    exprNode->op == oDIVIDE)
		  {
		    ...
		    /**  float  op  float   **/
		      ...
		      aqlError (aql, 896, exprNode->pos, "invalid expression types (%c).(%c) for arithmetic operator",
		...
		else if (exprNode->op == oMOD)
		  {
			...
			exprNode->vtype = 'i';
		      ...
		      aqlError (aql, 897, exprNode->pos, "invalid expression types (%c).(%c) for modulo operator (integer only)",
				exprNode->left->vtype, exprNode->right->vtype);
		  }
		else
    ...
    case nBOOL_EXISTS:
    {
	  ...
	  {
            indent(aql, level);
            aceOutPrint (aql->debug_out, "testing exists (exprNode->left=%lx && exprNode->left->loc=%lx)\n",
        ...
        if (exprNode->left && exprNode->left->loc)
	  {
            ...
            if (aql->evalDebugLevel >= 4) {
	      indent(aql, level);
	      aceOutPrint (aql->debug_out, "child_value = %s\n",
	...
	BOOL isValueLeft  = evalExpr(exprNode->left, isFuzzyRun, aql, level+1);
	BOOL isValueRight = evalExpr(exprNode->right, isFuzzyRun, aql, level+1);
  ...
  if (aql->evalDebugLevel >= 4)
    {
      indent(aql, level);
      aceOutPrint (aql->debug_out, "evalExpr result, isValue = %s:",
...
static BOOL stringComparison(AQL aql, AqlOpType boolOp, char *leftString, char *rightString, BOOL isCaseSensitive)
{
  ...
  switch (boolOp)
	...
	return (regExpMatch(leftString, rightString) > 0);
