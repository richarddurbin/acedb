 ...
 *
 * SCCS: $Id: aql_.h,v 1.20 2012-10-22 15:02:58 edgrif Exp $
 * Description:
 * Exported functions:
 * HISTORY:
...
struct AqlStruct {
  ...
  STORE_HANDLE handle;		        /* contains the actual object for query/errorJmpEnv/errorMessage and Report/checkEnv */
...
struct AqlQueryStruct {
  ...
  STORE_HANDLE	handle ;	/* handle in which all allocation for a query is done, 
...
void      aqlTraverse (AqlNode *p, 
                       ...
                       int level) ;
AqlQuery* aqlQueryCreate (const char *text, STORE_HANDLE objectHandle);
AqlScope* aqlScopeCreate (STORE_HANDLE handle);
