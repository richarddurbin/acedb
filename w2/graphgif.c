 ...
 * CVS info:   $Id: graphgif.c,v 1.14 2003-05-06 13:13:28 edgrif Exp $
 *-------------------------------------------------------------------
 */
...
{ 
  ...
  pixels = (unsigned char*) halloc(x*y*sizeof(unsigned char), graphClearHandle());

  for (j = 0 ; j < y ; ++j)
    for (p = pixels + j*x, i = 0 ; i < gdImageSX(im) ; ++i)
      *p++ = (unsigned char)im->pixels[i][j] ;
