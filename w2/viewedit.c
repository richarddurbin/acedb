...
static void viewWindowPick(int box, double x_unused, double y_unused, int modifier_unused);
...
void viewWindowCreate(void *dummy)
{
  ...
  if (getGraphAcedbDisplayCreate() != NULL &&
      (view->graph =
       (getGraphAcedbDisplayCreate())(getGraphAcedbViewName())))
    {
      if (!graphHelp(0))
	graphHelp ("Column-control"); /* default help unless otherwise 
    ...
    {
      view->graph = 
	graphCreate(TEXT_FIT, "Column Control", 0.2, 0.2, 0.8, 0.5);
      graphHelp ("Column-control");
    }
