 ...
 *
 * Exported functions:  see qbe.h
 * HISTORY:
 * Last edited: May  6 10:24 2003 (edgrif)
 ...
 * CVS info:   $Id: qbedisp.c,v 1.35 2003-05-06 13:13:15 edgrif Exp $
 *-------------------------------------------------------------------
 */
...
#include <ctype.h>
#include <wh/acedb.h>
#include <wh/bs.h>         /* used for bsTagsInClass() procedure */
#include <wh/query_.h>
#include <wh/query.h>
#include <wh/qbe.h>
#include <wh/bs_.h>        /* 10/12/92 gha */
#include <wh/lex.h>
#include <wh/key.h> 
...
#include <wh/display.h>	/* must come after QUER definition */
...
static void qbePick(int box, double x_unused, double y_unused, int modifier_unused);
...
{ 
  KEYSET ks = keySetCreate();	/* new,empty keyset */
  keySetNewDisplay(ks, "QBE Answer") ;
}
...
static void qbeMenuDraw(void)
{
  ...
  qbeDisplay () ;
  qbePick(qbelook->activebox, 0, 0, 0);
}
...
static BOOL  qbeEntryHandler (char *text, int unused)
{
  ...
  qbePick (nameBox, 0, 0, 0) ; /* mhmp 17.09.98 */
  /*  qbeShowQueryCommand(); mhmp 26.01.99*/
  graphActivate (old) ;
  return TRUE ;
}
...
static float qbeDrawBS (BS bs, int x, float y, int prev_x, float prev_y)
{
  ...
  if (bs->key <= _LastC)
    ...
    text = bs->n.time ?  timeShow (bs->n.time) : "" ;
  else
