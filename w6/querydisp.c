 ...
 **  queryCreate
 * HISTORY:
 * Last edited: May  6 13:55 2003 (edgrif)
 * * Jun  3 12:19 1996 (rd)
 ...
 */
 ...
 /* $Id: querydisp.c,v 1.26 2003-05-06 13:13:17 edgrif Exp $ */
...
static void querPick (int k, double x_unused, double y_unused, int modifier_unused) ;
...
static void qNewKeySet (void)
{
  ...
  keySetNewDisplay (ks, "Query Answer") ;
}
...
{ 
  ...
  if (arrayExists(array(quer->pgm,i,Array))) {
	...
	{ qbuildNewClass(class(keySet(newSet, 0)));
      ...
      querDisplay();
      querPick(quer->pgmBox+ 2*(i+1) + 1, 0, 0, 0);  /* go to next line */
    }
    else
...
void queryCommandUndo (void)
{
  ...
  if (keySetExists(query_undo_set) &&
    ...
    {
      keySetDestroy (oldSet) ;	/* destroys the look->kSet
				   of the active KeySet-window */
      newSet = keySetCopy(query_undo_set);
      if (newSet)    /* keySet should check for NULL; reset Taglist */
	qbuildNewClass(class(keySet(newSet, 0)));
      keySetShow (newSet, look) ; /* will eventually re-assign look->kSet
				     of the active window to newSet
...
void queryCreate (void)
{
  ...
  if ((cp = dbPathStrictFilName ("wquery", 0, 0, "rd", 0)))
    {
